...
@RestController
@RequestMapping("/places/kid-cafes")
@Slf4j
@AllArgsConstructor
public class PlaceKidCafeController {
    private PlaceKidCafeService placeKidCafeService;

    @GetMapping("")
    public ResponseEntity<?> index(PlaceCommonRequest placeCommonRequest){
        log.info("요청 옴!");
        System.out.println(placeCommonRequest);
        List<PlaceCommonResponse> placeCommonResponses =  placeKidCafeService.findAll(placeCommonRequest);
    ...
    public ResponseEntity<?> show(@PathVariable Long id){
        PlaceCommonResponse placeCommonResponse =  placeKidCafeService.findOne(id);
        Map<String, Object> response = new HashMap<>();
        response.put("message","데이터를 성공적으로 가져왔습니다.");
        response.put("statusCode", 200);
