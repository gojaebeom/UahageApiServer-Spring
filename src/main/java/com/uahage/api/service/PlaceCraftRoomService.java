...
import com.uahage.api.mapper.PlaceCraftRoomMapper;
...
@Slf4j
@Service
public class PlaceCraftRoomService {

    private PlaceCraftRoomMapper placeCraftRoomMapper;

    public List<PlaceCommonResponse> findAll(PlaceCommonRequest placeCommonRequest) {
        log.info("요청옴");
        if(placeCommonRequest.isMap()){
            return placeCraftRoomMapper.findMapByOptions(placeCommonRequest);
        }
        return placeCraftRoomMapper.findAllByOptions(placeCommonRequest);
    }

    public PlaceCommonResponse findOne(Long id) {
        return placeCraftRoomMapper.findOneById(id);
    }
}
